...
const Charts = {
  ...
  actualizarGraficoPrincipal(series, titulo, subtitulo, productosSeleccionados = []) {
    ...
    // Si no hay series, mostramos mensaje en la tabla y ocultamos sugerencias
    if (!series) {
      ...
      UI.actualizarTablaPrediccion([], []);        // ← fuerza el cartel de "sin datos"
      UI.actualizarSugerenciasCompra([], []);      // ← oculta sugerencias
      return;
    }

    const { historico, prediccion, conteoPorMes } = series;

    // Si la predicción viene vacía, también mostramos cartel
      ...
      options: {
        ...
        scales: {
          y: {
            ...
            title: { text: 'Cantidad (lts/kg)', color: isDark ? '#94a3b8' : '#64748b' }
          },
          x: {
    ...
    this.distribucionFamiliaChart = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          legend: {
            position: 'bottom',
            labels: { color: isDark ? '#cbd5e1' : '#334155', padding: 15 }
          }
        }
      }
    });
  ...
  _actualizarDistribucionActivo() {
    ...
    this.distribucionActivoChart = new Chart(ctx, {
      ...
      options: {
        ...
        scales: {
          x: { grid: { color: isDark ? '#334155' : '#e2e8f0' }, ticks: { color: isDark ? '#cbd5e1' : '#475569' } },
